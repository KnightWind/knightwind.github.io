...
public class Handler_Properties {
	...
	public static Properties loadProperties(String fileName, String dirName) {
		...
		try {
			int id = AppClass.getInstance().getResources().getIdentifier(fileName, dirName, AppClass.getInstance().getPackageName());
			props.load(AppClass.getInstance().getResources().openRawResource(id));
		} catch (Exception e) {
	...
	public static Properties loadConfigNoDirs(String fileName) {
		Properties properties = new Properties();
		try {
			FileInputStream s = AppClass.getInstance().openFileInput(fileName);
			properties.load(s);
		} catch (Exception e) {
			Util.BIZ_CONF_DEBUG(TAG, e.toString());
		}
	...
	public static void saveConfigNoDirs(String fileName, Properties properties) {
		try {
			FileOutputStream s = AppClass.getInstance().openFileOutput(fileName, Context.MODE_PRIVATE);
			properties.store(s, "");
		} catch (Exception e) {
			Util.BIZ_CONF_DEBUG(TAG, e.toString());
		}
	...
	public static Properties loadConfigAssets(String fileName) {
		...
		Properties properties = new Properties();
		try {
			InputStream is = AppClass.getInstance().getAssets().open(fileName);
			properties.load(is);
		} catch (Exception e) {
			Util.BIZ_CONF_DEBUG(TAG, e.toString());
		}
